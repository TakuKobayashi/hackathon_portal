module ItchIoOperation
  ...
  def self.imoport_gamejam_events!
    ...
    loop do
      ...
      events_dom = RequestParser.request_and_parse_html(
        url: root_uri.to_s,
        params: {
          page: page,
        },
      ...
      events_dom.css(".jam").each do |jam_dom|
        jam_dom.css(".jam_top_row").each do |jam_top_dom|
      ...
      event_url_set.each do |event_url|
        Event.transaction do
          ...
          hashtags = event_detail_dom.css(".jam_host_header").map{|d| d.css("a").map{|a| a.text }.select{|text| text.include?('#') } }.flatten
