...
module BackupToGoogleServices
  ...
  def self.backup_and_upload_and_clear_data!(backup_models: [])
    ...
    backup_models.each do |clazz|
      ...
      local_sql_file_path =
        Dumpdb.dump_table!(
          table_name: table_name, output_root_path: Rails.root.join('tmp').to_s
        )
      #s3 = Aws::S3::Client.new
      sql_filename =
        "#{Time.current.strftime('%Y%m%d_%H%M%S')}_#{table_name}.sql"
      result =
        drive.create_file(
  ...
  def self.clear_and_restart_table!(clazz:)
    ...
    clazz.connection.execute(
      "ALTER TABLE #{clazz.table_name} AUTO_INCREMENT=#{prev_auto_increment_id}"
    )
  end
    ...
    backup_models.each do |model_class|
      ...
      column_names =
        model_class.column_names -
          %w[created_at updated_at location_image_binary]
      start_row = 1
      start_column = 1
      ...
      if target_spreadsheet.sheets.all? do |sheet|
      ...
      range =
        "'#{sheet_name}'!R#{start_row}C#{start_column}:R#{end_row}C#{
          end_column
        }"
      cell_rows = []
      ...
      model_class.find_each do |event|
        cell_rows <<
          (column_names).map { |column_name| event.send(column_name).to_s }
      end
