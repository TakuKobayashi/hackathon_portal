...
module BackupToGoogleServices
  ...
  def self.backup_table_to_spreadsheet!(backup_models: [])
    ...
    backup_models.each do |model_class|
      ...
      column_names = model_class.column_names - %w[created_at updated_at location_image_binary]
      start_row = 1
      start_column = 1
      ...
      model_class.find_each { |event| cell_rows << (column_names).map { |column_name| event.send(column_name).to_s } }
