...
class Scaling::DoorkeeperUnityEvent < Scaling::UnityEvent
  ...
  def self.import_events!
    ...
    while events_response.present?
      begin
        events_response = self.find_event(keywords: Event::HACKATHON_KEYWORDS + %w[はっかそん], page: page)
        current_events =
          Scaling::DoorkeeperUnityEvent.where(event_id: events_response.map { |res| res['event']['id'] }.compact).index_by(&:event_id)
        transaction do
