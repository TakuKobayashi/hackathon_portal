...
class Scaling::DoorkeeperUnityEvent < Scaling::UnityEvent
  ...
  def self.import_events!
    page = 1
    begin
      events_response = self.find_event(keywords: Event::HACKATHON_KEYWORDS + ["はっかそん"], page: page)
      current_events = Scaling::DoorkeeperUnityEvent.where(event_id: events_response.map { |res| res["event"]["id"] }.compact).index_by(&:event_id)
      transaction do
        events_response.each do |res|
          ...
          if current_events[event["id"].to_s].present?
            ...
            doorkeeper_event = Scaling::DoorkeeperUnityEvent.new(event_id: event["id"].to_s)
          end
