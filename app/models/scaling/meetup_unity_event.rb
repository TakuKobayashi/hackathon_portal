...
class Scaling::MeetupUnityEvent < Scaling::UnityEvent
  ...
  def self.import_events!
    update_columns = self.column_names - ["id", "type", "shortener_url", "event_id", "created_at"]
    events_response = self.find_event(keywords: Scaling::UnityEvent::UNITY_KEYWORDS)
    current_events = Scaling::MeetupUnityEvent.where(event_id: events_response["events"].map { |res| res["id"] }.compact).index_by(&:event_id)
    transaction do
      events_response["events"].each do |res|
        if current_events[res["id"].to_s].present?
          meetup_event = current_events[res["id"].to_s]
        else
          meetup_event = Scaling::MeetupUnityEvent.new(event_id: res["id"].to_s)
        end
