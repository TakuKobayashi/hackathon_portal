...
class Scaling::AtndUnityEvent < Scaling::UnityEvent
  ...
  def self.import_events!
    start = 1
    begin
      events_response = self.find_event(keywords: Scaling::UnityEvent::UNITY_KEYWORDS, start: start)
      start += events_response["results_returned"]
      current_events = Scaling::AtndUnityEvent.where(event_id: events_response["events"].map{|res| res["event"]["event_id"]}.compact).index_by(&:event_id)
      transaction do
        events_response["events"].each do |res|
          ...
          if current_events[event["event_id"].to_s].present?
            ...
            atnd_event = Scaling::AtndUnityEvent.new(event_id: event["event_id"].to_s)
          end
