...
class BloggerBot < ApplicationRecord
  ...
  def self.post_or_update_article!(events: [], event_type: "Event")
    ...
    events_group.each do |date_number, event_arr|
      ...
      blogger_bot.body = action_view_renderer.render(template: 'blogger/publish', format: 'html', locals: { before_events: before_events, after_events: after_events, date_number: date_number })
